...
public class SpotAccountTradeGetApiV3AccountTradeList extends SAProxy {
	public DataSet GetApiV3AccountTradeList(DataSet InDs, String InDsNames, String outDsNames) throws Exception {
		String URL = "https://api.binance.com/api/v3/myTrades";
		...
		OUT_RSET.addColumn("ID", dataset.type.DataType.STRING, null, "id");
		...
		OUT_RSET.addColumn("QTY", dataset.type.DataType.STRING, null, "qty");
		OUT_RSET.addColumn("QUOTE_QTY", dataset.type.DataType.STRING, null, "quoteQty");
		OUT_RSET.addColumn("COMMISSION", dataset.type.DataType.STRING, null, "commission");
		OUT_RSET.addColumn("COMMISSION_ASSET", dataset.type.DataType.STRING, null, "commissionAsset");

		OUT_RSET.addColumn("TIME", dataset.type.DataType.STRING, null, "time");
		OUT_RSET.addColumn("IS_BUYER", dataset.type.DataType.STRING, null, "isBuyer");
		OUT_RSET.addColumn("IS_MAKER", dataset.type.DataType.STRING, null, "isMaker");
		OUT_RSET.addColumn("IS_BEST_MATCH", dataset.type.DataType.STRING, null, "isBestMatch");
		...
		if (PjtUtil.g().isEmpty(SYMBOL)) {
			...
			drRst.setString("ERR_MSG", "SYMBOL가  인풋으로 넘어오지 않았습니다.");
		...
		try {
			...
			for (int i = 0; i < al.size(); i++) {
				...
				DR_OUT_RSET.setString("ID", c.get("id").toString());
				...
				DR_OUT_RSET.setString("QTY", c.get("qty").toString());
				DR_OUT_RSET.setString("QUOTE_QTY", c.get("quoteQty").toString());
				DR_OUT_RSET.setString("COMMISSION", c.get("commission").toString());
				DR_OUT_RSET.setString("COMMISSION_ASSET", c.get("commissionAsset").toString());

				DR_OUT_RSET.setString("TIME", c.get("time").toString());
				DR_OUT_RSET.setString("IS_BUYER", c.get("isBuyer").toString());
				DR_OUT_RSET.setString("IS_MAKER", c.get("isMaker").toString());
				DR_OUT_RSET.setString("IS_BEST_MATCH", c.get("isBestMatch").toString());
			}
