...
public class WalletEndPointsGetSapiV1DailyAccountSnapshotMARGIN extends SAProxy {
	// https://binance-docs.github.io/apidocs/spot/en/#all-coins-39-information-user_data
	public DataSet GetSapiV1DailyAccountSnapshotMARGIN(DataSet InDs, String InDsNames, String outDsNames)
			throws Exception {
		...
		OUT_RSET.addColumn("MARGIN_LEVEL");
		OUT_RSET.addColumn("TOTAL_ASSET_OF_BTC");
		OUT_RSET.addColumn("TOTAL_LIABILITY_OF_BTC");
		OUT_RSET.addColumn("TOTAL_NET_ASSET_OF_BTC");

		DataTable OUT_RSET_DATA__USER_ASSETS = OUT_DS.addTable("OUT_RSET_DATA__USER_ASSETS");
		OUT_RSET_DATA__USER_ASSETS.addColumn("SEQ");
		OUT_RSET_DATA__USER_ASSETS.addColumn("ASSET");
		OUT_RSET_DATA__USER_ASSETS.addColumn("BORROWED");
		OUT_RSET_DATA__USER_ASSETS.addColumn("FREE");
		OUT_RSET_DATA__USER_ASSETS.addColumn("INTEREST");
		OUT_RSET_DATA__USER_ASSETS.addColumn("LOCKED");
		OUT_RSET_DATA__USER_ASSETS.addColumn("NET_ASSET");
			...
			if (c.get("snapshotVos") != null) {
				...
				for (int i = 0; i < al.size(); i++) {
					...
					if (tmp.get("data") != null) {
						...
						DR_OUT_RSET.setString("MARGIN_LEVEL", tmp2.get("marginLevel").toString());
						DR_OUT_RSET.setString("TOTAL_ASSET_OF_BTC", tmp2.get("totalAssetOfBtc").toString());
						DR_OUT_RSET.setString("TOTAL_LIABILITY_OF_BTC", tmp2.get("totalLiabilityOfBtc").toString());
						DR_OUT_RSET.setString("TOTAL_NET_ASSET_OF_BTC", tmp2.get("totalNetAssetOfBtc").toString());

						if (tmp2.get("userAssets") != null) {
							ArrayList<HashMap<String, Object>> al2 = (ArrayList<HashMap<String, Object>>) tmp2
									.get("userAssets");
							for (int j = 0; j < al2.size(); j++) {
								HashMap<String, Object> tmp3 = al2.get(j);
								DataRow DR_OUT_RSET_DATA__USER_ASSETS = OUT_RSET_DATA__USER_ASSETS.addRow();
								DR_OUT_RSET_DATA__USER_ASSETS.setString("SEQ", i + "");
								DR_OUT_RSET_DATA__USER_ASSETS.setString("ASSET", tmp3.get("asset").toString());
								DR_OUT_RSET_DATA__USER_ASSETS.setString("BORROWED", tmp3.get("borrowed").toString());
								DR_OUT_RSET_DATA__USER_ASSETS.setString("FREE", tmp3.get("free").toString());
								DR_OUT_RSET_DATA__USER_ASSETS.setString("INTEREST", tmp3.get("interest").toString());
								DR_OUT_RSET_DATA__USER_ASSETS.setString("LOCKED", tmp3.get("locked").toString());
								DR_OUT_RSET_DATA__USER_ASSETS.setString("NET_ASSET", tmp3.get("netAsset").toString());
							}
						}
					}
				}
