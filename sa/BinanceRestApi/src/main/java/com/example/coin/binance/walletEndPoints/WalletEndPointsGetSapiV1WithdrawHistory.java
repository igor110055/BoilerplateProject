...
public class WalletEndPointsGetSapiV1WithdrawHistory extends SAProxy {
	// https://binance-docs.github.io/apidocs/spot/en/#all-coins-39-information-user_data
	public DataSet GetSapiV1WithdrawHistory(DataSet InDs, String InDsNames, String outDsNames) throws Exception {
		...
		OUT_RSET.addColumn("APPLY_TIME", dataset.type.DataType.STRING, null, "applyTime");
		OUT_RSET.addColumn("COIN", dataset.type.DataType.STRING, null, "coin");
		OUT_RSET.addColumn("ID", dataset.type.DataType.STRING, null, "id");
		OUT_RSET.addColumn("WITHDRAW_ORDER_ID", dataset.type.DataType.STRING, null, "withdrawOrderId");
		...
		OUT_RSET.addColumn("TRANSACTION_FEE", dataset.type.DataType.STRING, null, "transactionFee");
		OUT_RSET.addColumn("CONFIRM_NO", dataset.type.DataType.STRING, null, "confirmNo");
		...
		String STATUS = null;// 0(0:Email Sent,1:Cancelled 2:Awaiting Approval 3:Rejected 4:Processing
		...
		if (dt.getRowCount() > 0) {
			...
			WITHDRAW_ORDER_ID = dt.getRow(0).getStringNullToEmpty("WITHDRAW_ORDER_ID");
		...
		if (!PjtUtil.g().isEmpty(WITHDRAW_ORDER_ID)) {
			params.put("withdrawOrderId", WITHDRAW_ORDER_ID);
		}
		...
		for (int i = 0; i < al.size(); i++) {
			...
			dr.setString("TRANSACTION_FEE", c.get("transactionFee").toString());

			dr.setString("CONFIRM_NO", c.get("confirmNo").toString());
