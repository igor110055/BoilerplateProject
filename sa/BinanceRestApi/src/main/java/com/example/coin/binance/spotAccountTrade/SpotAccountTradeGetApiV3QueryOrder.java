...
public class SpotAccountTradeGetApiV3QueryOrder extends SAProxy {
	public DataSet GetApiV3QueryOrder(DataSet InDs, String InDsNames, String outDsNames) throws Exception {
		...
		if (PjtUtil.g().isEmpty(ORDER_ID) && PjtUtil.g().isEmpty(ORIG_CLIENT_ORDER_ID)) {
			// 에러처리
			drRst.setString("JSON_OUT", "");
			drRst.setString("STATUS", "E");
			drRst.setString("ERR_MSG", "ORDER_ID 또는 ORIG_CLIENT_ORDER_ID 중 하나는 인풋으로 넘어와야합니다.(2개 모두 넘어오지 않았음)");
