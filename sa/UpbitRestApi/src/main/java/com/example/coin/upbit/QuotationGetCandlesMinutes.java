...
public class QuotationGetCandlesMinutes extends SAProxy {

    public DataSet GetCandlesMinutes(DataSet InDs,String InDsNames, String outDsNames)  throws Exception {
    	...
    	OUT_RSET.addColumn("MARKET");//마켓명
    	OUT_RSET.addColumn("CANDLE_DATE_TIME_UTC");//캔들 기준 시각(UTC 기준)
    	OUT_RSET.addColumn("CANDLE_DATE_TIME_KST");//캔들 기준 시각(KST 기준)
    	OUT_RSET.addColumn("OPENING_PRICE");//시가
    	OUT_RSET.addColumn("HIGH_PRICE");//고가
    	OUT_RSET.addColumn("LOW_PRICE");//저가
    	OUT_RSET.addColumn("TRADE_PRICE");//종가
    	OUT_RSET.addColumn("TIMESTAMP");//마지막 틱이 저장된 시각
    	OUT_RSET.addColumn("CANDLE_ACC_TRADE_PRICE");//누적 거래 금액
    	OUT_RSET.addColumn("CANDLE_ACC_TRADE_VOLUME");//누적 거래량
    	OUT_RSET.addColumn("UNIT");//	분 단위(유닛)	Integer
    	...
    	if(IN_PSET.getRowCount()>0) {
    		...
    		UNIT = IN_PSET.getRow(0).getStringNullToEmpty("UNIT");
    	}
    	URL = URL+UNIT;
	    ...
	    if(PjtUtil.g().isEmpty(UNIT)){
            //에러처리
			drRst.setString("JSON_OUT", "");
			drRst.setString("STATUS", "E");
			drRst.setString("ERR_MSG", "UNIT가 인풋으로 넘어오지 않았습니다.");
		...
		for(int i=0;i<al.size();i++) {
			...
			dr.setString("OPENING_PRICE",  PjtUtil.g().doubleToString(Double.parseDouble(c.get("opening_price").toString()))   );
			dr.setString("HIGH_PRICE",  PjtUtil.g().doubleToString(Double.parseDouble(c.get("high_price").toString()))   );
			dr.setString("LOW_PRICE",  PjtUtil.g().doubleToString(Double.parseDouble(c.get("low_price").toString()))   );
			
			dr.setString("TRADE_PRICE",  PjtUtil.g().doubleToString(Double.parseDouble(c.get("trade_price").toString()))   );
			dr.setString("TIMESTAMP",c.get("timestamp").toString());
			dr.setString("CANDLE_ACC_TRADE_PRICE",  PjtUtil.g().doubleToString(Double.parseDouble(c.get("candle_acc_trade_price").toString()))   );
			dr.setString("CANDLE_ACC_TRADE_VOLUME",  PjtUtil.g().doubleToString(Double.parseDouble(c.get("candle_acc_trade_volume").toString()))   );

			dr.setString("UNIT",c.get("unit").toString());
