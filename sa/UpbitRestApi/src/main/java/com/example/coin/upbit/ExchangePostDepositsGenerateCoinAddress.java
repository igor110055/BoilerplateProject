...
public class ExchangePostDepositsGenerateCoinAddress extends SAProxy {

    public DataSet PostDepositsGenerateCoinAddress(DataSet InDs,String InDsNames, String outDsNames)  throws Exception {
    	String  URL="https://api.upbit.com/v1/deposits/generate_coin_address";
    	...
    	DataTable OUT_RSET1 = OUT_DS.addTable("OUT_RSET1");
    	OUT_RSET1.addColumn("SUCCESS");//요청 성공 여부
    	OUT_RSET1.addColumn("MESSAGE");//요청 결과에 대한 메세지
    	
    	DataTable OUT_RSET2 = OUT_DS.addTable("OUT_RSET2");
    	OUT_RSET2.addColumn("CURRENCY");//	화폐를 의미하는 영문 대문자 코드
    	OUT_RSET2.addColumn("DEPOSIT_ADDRESS");//입금 주소
    	OUT_RSET2.addColumn("SECONDARY_ADDRESS");//2차 입금 주소
		...
		if(c!=null) {
			if(c.get("success")!=null){
				DataRow dr = OUT_RSET1.addRow();
				dr.setString("SUCCESS",c.get("success").toString());
				dr.setString("MESSAGE",c.get("message").toString());
			} else {
				DataRow dr = OUT_RSET2.addRow();
