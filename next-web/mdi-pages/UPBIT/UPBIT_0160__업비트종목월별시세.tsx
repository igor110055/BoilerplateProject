...
export const UPBIT_0160__업비트종목월별시세= () => {   
    ...
    const fnSearch= async (data: IFormInput) => {
        if(childRef.current){
            childRef.current.loadData({
                                        BR:"BR_UPBIT_CANDLES_MONTHS_FIND",
                                        PARAM: {
                                            brRq : 'IN_DATA'
                                            ,brRs : 'OUT_DATA'
                                            ,IN_DATA:[data]
    ...
    const syncHandler= async (data: IFormInput) => {
        ...
        messageConfirm("싱크하시겠습니까?",function()  {
            //_this.showProgress();	
            send('BR_UPBIT_CANDLES_MONTHS_SYNC',{
                brRq 		: 'IN_DATA'
                ,brRs 		: ''
                ,IN_DATA:[ data ]
            }).then(function(data){
    ...
    return (
            ...
            <Stack
                ...
                >
                    ...
                    <Button variant="contained" color="success" onClick={handleSubmit(syncHandler)}>월별 싱크(30개월 이전)</Button>
                    업비트는 오전 9시에 종가 초기화
            </Stack>
